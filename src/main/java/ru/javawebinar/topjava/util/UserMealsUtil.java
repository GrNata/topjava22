    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal meal : meals) {
            if (map.get(meal.getDateTime().toLocalDate()) != null) {
                caloria = map.get(meal.getDateTime().toLocalDate()) + meal.getCalories();
            } else {
                caloria = meal.getCalories();
            }
            map.put(meal.getDateTime().toLocalDate(), caloria);
        }
        ...
        for (UserMeal meal : meals) {
            if (meal.getDateTime().toLocalTime().isAfter(startTime) &&
                ...
                UserMealWithExcess userMeal = new UserMealWithExcess(meal.getDateTime(),
                                                                     meal.getDescription(),
                                                                     meal.getCalories(),
                                                              map.get(meal.getDateTime().toLocalDate()) > caloriesPerDay ? true : false);
//                true - превышает, false - не превышает
                mealWithExcesses.add(userMeal);
            }
        }
